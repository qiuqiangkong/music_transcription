...
from .io import read_single_track_midi, notes_to_rolls_and_events, pedals_to_rolls_and_events, events_to_notes, notes_to_midi, fix_length, time_to_grid
...
class Maestro:
    ...
    def __getitem__(self, index):
        ...
        data = {
            ...
            "segment_start_time": segment_start_time,
            "audio": audio,
            "frame_roll": targets_dict["frame_roll"][None, ...].astype(np.float32),  # (s, t, k)
            "onset_roll": targets_dict["onset_roll"][None, ...].astype(np.float32),  # (s, t, k)
            "offset_roll": targets_dict["offset_roll"][None, ...].astype(np.float32),  # (s, t, k)
            "velocity_roll": targets_dict["velocity_roll"][None, ...].astype(np.float32),  # (s, t, k)
            # "event": targets_dict["event"],
            # "string": targets_dict["string"],
            "token": np.array(targets_dict["token"])[None, ...],  # (s, l)
            "token_mask": np.ones(targets_dict["tokens_num"], dtype=np.bool_)[None, ...],  # (s, l)
            "tokens_num": np.array([targets_dict["tokens_num"]]),
